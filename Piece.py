"""
This module defines the basic components of a Rubik's Cube puzzle, including various types of pieces such as
internal, middle, edge, and corner pieces. Each piece has unique identifiers and can undergo transformations
like rotations, reflecting their behavior in a physical Rubik's Cube.

Classes:
    Piece: Base class for a piece of the Rubik's Cube.
    InternalPiece: Represents an internal (hidden) piece that does not move.
    MiddlePiece: Represents a middle piece that can be part of two sides but doesn't reach any edge.
...
class Piece:
    """
    Represents a piece of a Rubik's Cube. Each piece has unique properties such as type and face colors.
    Pieces can be rotated, reflecting their behavior in a physical Rubik's Cube.
    ...
    Attributes:
        ...
        faces (dict): A dictionary mapping each face ('U', 'L', 'F', 'R', 'B', 'D') to its color.
        rotationMap (dict): A dictionary mapping rotation directions to cube faces.

    Methods:
        rotate: Rotates the piece according to a specified rotation direction and mode (prime).
        doRotation: Performs the rotation on the piece's faces based on the specified axis.
    ...
    def rotate(self, rotation:str, prime:bool=False) -> None:
        ...
        Parameters:
            ...
            prime (bool): Indicates whether the rotation should be counter-clockwise (True) or clockwise (False).
        """
    ...
    def doRotation(self, axis:str, prime:bool=False) -> None:
        ...
        Parameters:
            ...
            prime (bool): Indicates if the rotation should be counter-clockwise (True) or clockwise (False).
        """
        if axis == "X":
